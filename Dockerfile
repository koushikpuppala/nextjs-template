...
RUN apk add --no-cache curl bash && \
    ...
    addgroup --system --gid 1001 template && \
    # Create a new system user with UID 1001 and add it to the "template" group
    adduser --system --uid 1001 next --ingroup template && \
...
# Change the ownership of the .next, public, and logs directory to the "next" user and "template" group
RUN chown -R next:template .next public logs
